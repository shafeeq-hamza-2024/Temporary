    ...
    <div className="bg-white rounded-3xl border border-gray-100 overflow-hidden shadow-xs hover:shadow-md transition-shadow duration-300">
      ...
      <div className="flex items-start justify-between p-5 sm:p-6">
        <div className="flex items-center gap-4 min-w-0">
          {/* Page Avatar */}
          <div
            className="w-12 h-12 sm:w-14 sm:h-14 rounded-2xl bg-gray-50 overflow-hidden shrink-0 shadow-sm border border-gray-100 cursor-pointer transform hover:scale-105 transition-transform duration-200"
          ...
          >
            {page?.profile_image_url ? (
              ...
              <div className="w-full h-full bg-linear-to-br from-primary/10 to-primary/20 flex items-center justify-center text-primary font-black text-xl">
          ...
          <div className="flex flex-col min-w-0">
            <div className="flex items-center gap-2 leading-tight">
              <span
                className="text-[15px] sm:text-lg font-black text-gray-900 truncate cursor-pointer hover:text-primary transition-colors tracking-tight"
                onClick={() => page?.id && navigate(`/pages/${page.id}`)}
              >
                {page?.page_name || "Page Post"}
              </span>
              <MdVerifiedUser className="text-primary text-base shrink-0" title="Verified Page" />
            </div>

            <div className="flex items-center gap-2 mt-1 px-1">
               <div className="flex items-center gap-1.5 cursor-pointer group/user" onClick={() => navigate(`/public/users/${post.created_by.id}`)}>
                  <span className="text-[12px] font-bold text-gray-500 group-hover:text-primary transition-colors">
                    {post.created_by.first_name} {post.created_by.last_name}
                  </span>
                  <span className="text-[10px] font-black text-blue-500 uppercase tracking-widest bg-blue-50 px-1.5 py-0.5 rounded-md border border-blue-100/50">
                    Admin
                  </span>
               </div>
